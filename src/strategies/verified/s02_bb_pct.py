"""WF+BB%B noTP: walk-forward + %B position filter, no take-profit"""
import numpy as np

from src.evaluation.engine import run_backtest_custom
from src.evaluation.simulation import get_tp_sl_no_tp
from src.signals.filters import bb_pct_ok, compute_bb
...
def run(data, verbose=True):
    ...
    for i in range(data['N']):
        if base[i] == 0:
            continue
        if bb_pct_ok(i, base[i], bb_pct_vals):
            sig[i] = base[i]
    return run_backtest_custom(sig, "WF+BB%B noTP", get_tp_sl_no_tp, data, pred_ret_ref=data['pred_ret'], verbose=verbose)
